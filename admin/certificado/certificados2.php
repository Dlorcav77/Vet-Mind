...
function obtenerDatosPaciente() {
  ...
  if (esManual) {
    ...
    // selects manuales
    const razaId  = ($('#manual_raza').val() || '').trim();
    const sexoVal = ($('#manual_sexo').val() || '').trim();

    if (razaId) {
      const $opt     = $('#manual_raza').find('option:selected');
      const razaNom  = ($opt.text() || '').trim();
      const especie  = ($opt.closest('optgroup').attr('label') || '').trim();
      datos['raza_id'] = razaId;
      datos['raza']    = razaNom;            // por compatibilidad con el backend
      datos['especie'] = datos['especie'] || especie;
      $('#manual_raza_nombre').val(razaNom); // sincroniza hidden
      $('#manual_especie').val(especie);     // sincroniza hidden
    }
