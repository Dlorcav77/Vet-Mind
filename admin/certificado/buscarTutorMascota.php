...
$query = "SELECT t.id as tutor_id, t.nombre_completo as tutor_nombre, t.rut, 
                p.id as paciente_id, p.nombre as mascota_nombre, p.especie, p.chip
          FROM tutores t
...
$stmt->bind_param("issssi", $usuario_id, $q, $q, $q, $q, $usuario_id);
$stmt->execute();
$res = $stmt->get_result();
...
while ($row = $res->fetch_assoc()) {
    echo "<tr>
            ...
            <td>" . resaltar(htmlspecialchars($row['chip'] ?? '-'), $q) . "</td>
            <td>" . htmlspecialchars($row['especie'] ?? '-') . "</td>
            <td>
              <button class=\"btn btn-sm btn-primary\" 
                      onclick=\"seleccionarPaciente(
                        {$row['paciente_id']}, 
                        '".addslashes($row['mascota_nombre'])."', 
                        '".addslashes($row['tutor_nombre'])."')\">
                Seleccionar
              </button>
            </td>
          </tr>";
