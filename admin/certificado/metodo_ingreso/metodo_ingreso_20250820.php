...
<div class="d-flex justify-content-between align-items-center mb-2">
  ...
  <div id="audio_manual_segmented" class="btn-group" role="group" aria-label="Modo de ingreso">
    <button type="button" class="btn btn-outline-info active" id="audio_manual_audioBtn">🎤 Audio</button>
    <button type="button" class="btn btn-outline-info" id="audio_manual_manualBtn">📝 Manual</button>
  </div>

  <!-- Botón de procesar (el texto cambia según el modo) -->
  <button type="button" class="btn btn-success btn-lg rounded-pill shadow-sm px-4" id="procesarIA">
    ✨ Procesar IA
  </button>
...
<!-- Bloque Subir Audio -->
<div id="bloque-audio" class="col-12">
    <div class="card card-audio-dark border-0 shadow-lg rounded-4 p-1 audio-center-wrap mx-auto ">
        ...
        <div class="tab-content" id="audioTabContent">
            ...
            <div class="tab-pane fade" id="subir" role="tabpanel" aria-labelledby="subir-tab">
                <input type="file" class="form-control mb-2" name="archivo_audio">
                <small class="form-text text-muted">Puede subir un archivo de audio en formato MP3, WAV o WEBM.</small>
            </div>
        </div>
    </div>
</div>
<div id="bloque-manual" class="col-12 mb-1" style="display:none;">
...
(() => {
    ...
    window.startRecording = function () {
        ...
        .then(stream => {
            ...
            // El resto de tu lógica, timer, etc.
            window.seconds = 0;
            $('#recordingStatus').html('🎙 <strong>Grabando...</strong> <span id="timer" class="text-danger fw-bold">00:00</span>');
    ...
    window.stopRecording = function () {
        ...
        window.recorder.onstop = () => {
            // tu lógica para guardar o mostrar el audio
            // por ejemplo:
            if (window.audioBlob) {
                let formData = new FormData();
                formData.append('audio', window.audioBlob, 'grabacion.webm');
                fetch('/funciones/guardar_audio.php', {
                    method: 'POST',
                    body: formData
                })
                ...
                .then(data => {
                    if (data.status === 'success') {
                        ...
                        $('#audioInfo').html(`
                            <div class="mt-0">
                                <button class="btn btn-sm btn-danger ms-2" onclick="deleteRecording()">
                                    🗑 Eliminar
                                </button>
                            </div>
                        `);
...
function audio_manual_setMode(mode) {
  ...
  // 1) botones del segmented
  $('#audio_manual_audioBtn').toggleClass('active', !toManual);
  $('#audio_manual_manualBtn').toggleClass('active', toManual);

  // 2) compatibilidad: refleja el estado en el hidden
  $('#toggle_audio_manual').prop('checked', toManual);

  // 3) mostrar/ocultar bloques + CKEditor
  if (toManual) {
    $('#bloque-audio').slideUp(120);
    $('#bloque-manual').slideDown(120, function () {
...
$(function () {
  ...
  // Modo inicial: audio para "ingresar", manual para "modificar"
  const initialMode = <?= (isset($action) && $action === 'modificar') ? "'manual'" : "'audio'"; ?>;
  audio_manual_setMode(initialMode);
});
