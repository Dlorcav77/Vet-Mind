...
<style>
  ...
  #campo-select option {
      ...
      font-weight: 600;        /* Un poquito más grueso */
  }
...
<div class="card" id="configuracion_informe" data-page-id="configuracion_informe">
  <div class="card-header">
    <h1 class="h3 mb-3"><strong><?= $accion ?> Configuración de Informes</strong></h1>
  </div>
  <div class="card-body">
              ...
              <div class="row">
                <div class="col-md-6 mb-2">
                  ...
                  <select id="logo_position" name="logo_position" class="form-select select2">
                    <option value="left" <?= $fila['logo_position'] === 'left' ? 'selected' : '' ?> data-icon="fas fa-align-left">Izquierda</option>
                    <option value="center" <?= $fila['logo_position'] === 'center' ? 'selected' : '' ?> data-icon="fas fa-align-center">Centro</option>
                    <option value="right" <?= $fila['logo_position'] === 'right' ? 'selected' : '' ?> data-icon="fas fa-align-right">Derecha</option>
                  </select>
                </div>
                <div class="col-md-6 mb-2">
              ...
              <div class="row">
                <div class="col-md-6 mb-2">
                  ...
                  <div class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" name="mostrar_marca_agua" value="1" <?= $fila['mostrar_marca_agua'] ? 'checked' : '' ?>>
...
<div class="tab-pane fade" id="campos" role="tabpanel">
  ...
  <table class="table table-bordered">
    ...
    <tbody id="campos-lista">
      ...
      ");
      $stmt_campos->bind_param("i", $_SESSION['usuario_id']);
      $stmt_campos->execute();
  ...
  <!-- Select y botón para agregar campos -->
  ...
  <div class="border p-3 rounded">
    <table class="table table-bordered mb-0 bg-light">
      <tbody id="vista-previa-campos">
        <?php
        $contador = 0;
        echo '<tr>';
        foreach ($result_campos as $campo):
            $contador++;
            ?>
            <th style="width: 15%; white-space: nowrap;"><?= htmlspecialchars($campo['etiqueta']) ?>:</th>
        ...
        <div class="tab-pane fade" id="colores" role="tabpanel">
          ...
          <div class="card mt-3 p-3">
            ...
            <div class="d-flex">
              <div class="flex-fill text-center p-2" style="background-color: <?= htmlspecialchars($fila['color_primario']); ?>; color: #fff;">
                Color Primario
              </div>
              <div class="flex-fill text-center p-2" style="background-color: <?= htmlspecialchars($fila['color_secundario']); ?>; color: #fff;">
                Color Secundario
              </div>
            </div>
          </div>
        ...
        <div class="tab-pane fade" id="firma" role="tabpanel">
          <div class="row mb-3">
            <div class="col-md-6 mb-2">
              ...
              <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" name="mostrar_firma_imagen" value="1" <?= ($fila['mostrar_firma_imagen'] ?? 0) ? 'checked' : '' ?>>
                <label class="form-check-label">Activar</label>
              </div>
            </div>
            <div class="col-md-6 mb-2">
          ...
          <div class="row mb-3">
            ...
            <div class="col-md-12 mb-2">
              <label for="firma_subtitulo" class="form-label">Subtítulo</label>
              <input type="text" class="form-control" name="firma_subtitulo" maxlength="150"
                    value="<?= htmlspecialchars($fila['firma_subtitulo'] ?? '') ?>" placeholder="Ej: N° Registro CMV 12345">
            </div>
            <div class="col-md-6 mb-2">
        ...
        <div class="tab-pane fade" id="fecha" role="tabpanel">
          <div class="row mb-3">
            <div class="col-md-6 mb-2">
              ...
              <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" name="mostrar_fecha" value="1" <?= ($fila['mostrar_fecha'] ?? 0) ? 'checked' : '' ?>>
                <label class="form-check-label">Activar</label>
              </div>
            </div>
            <div class="col-md-6 mb-2">
...
$('form').on('submit', function(e) {
    ...
    // ✅ Agrega un campo oculto con el orden actualizado
    if ($('#campos_orden').length === 0) {
        $('<input>').attr({
            type: 'hidden',
            id: 'campos_orden',
            name: 'campos_orden'
...
$(function() {
  ...
  $(document).on("click", ".eliminar-campo-nuevo", function() {
    $(this).closest('tr').remove();
  });
});
...
function actualizarVistaPrevia() {
    ...
    for (let i = 0; i < campos.length; i += 2) {
        html += '<tr>';
        html += `<th style="width: 15%;">${campos[i]}:</th><td style="width: 35%;"></td>`;
        if (campos[i + 1]) {
            html += `<th style="width: 15%;">${campos[i + 1]}:</th><td style="width: 35%;"></td>`;
        } else {
...
$(document).on('input change', '#campos-lista input', actualizarVistaPrevia);
$(document).on('click', '#agregar-campo, .eliminar-campo, .eliminar-campo-nuevo', function () {
    setTimeout(actualizarVistaPrevia, 100); // Espera a que el DOM se actualice
});
...
$(document).ready(function () {
  ...
  $('#agregar-campo').on('click', function () {
    ...
    // Actualiza select y vista previa
    actualizarOpcionesSelect();
    actualizarVistaPrevia();
  });
...
function actualizarOpcionesSelect() {
  // Habilita todas las opciones al inicio
  $('#campo-select option').prop('disabled', false);

  // Deshabilita opciones de campos ya agregados
  $('#campos-lista tr').each(function () {
    const campoId = $(this).data('campo-id');
    $('#campo-select option[value="' + campoId + '"]').prop('disabled', true);
  });

  // Si el seleccionado actual está deshabilitado, mueve el select al primer habilitado
  if ($('#campo-select option:selected').prop('disabled')) {
    let firstEnabled = $('#campo-select option:not(:disabled)').first();
    $('#campo-select').val(firstEnabled.val());
  }
}
