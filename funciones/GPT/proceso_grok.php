...
if ($result === null) {
    echo json_encode(['status' => 'error', 'message' => 'Respuesta inválida de xAI/Grok (JSON).', 'rid' => $rid]);
    exit;
}

if ($http_code !== 200) {
    $err_detail = $result['error']['message']
        ?? $result['message']
        ?? $result['detail']
        ?? ('HTTP ' . $http_code);
    ...
    echo json_encode([
        'status'     => 'error',
        'message'    => 'Error API xAI/Grok: ' . $err_detail,
        'rid'        => $rid,
        'http_code'  => $http_code,
        'api_detail' => $result,
...
// xAI puede devolver costo exacto en ticks.
// 1 USD = 10.000.000.000 ticks.
$cost_usd = 0.0;
if (isset($usage['cost_in_usd_ticks'])) {
    $cost_usd = round(((float)$usage['cost_in_usd_ticks']) / 10_000_000_000, 6);
}

if ($cost_usd <= 0) {
    $cost_usd = gpt_estimate_cost_usd(GROK_MODEL, $prompt_tokens, $completion_tokens);
}
...
if (GPT_SNAPSHOT === 1) {
    ...
    $snapshot = [
        ...
        'usage'         => [
            ...
            'total_tokens'      => $total_tokens,
            'cost_usd'          => $cost_usd,
            'cost_in_usd_ticks' => $usage['cost_in_usd_ticks'] ?? null,
        ],
    ];
